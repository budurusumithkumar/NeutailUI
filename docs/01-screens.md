...
- **Content:** grid of t-shirt thumbnails (color/pattern only, no photos — rendered as inline SVG); a preview panel (`CameraTryOn.tsx`) that requests the device camera via `getUserMedia`, runs client-side body-part segmentation each frame (TensorFlow.js BodyPix — `bodySegmentation.ts`) to find which pixels are the customer's actual torso/upper-arms, and recolors *that existing clothing* to the selected tee's color/pattern via the canvas `color` composite mode (`drawGarmentFill.ts`) — this keeps the customer's real folds, shading, and motion instead of warping a synthetic shape over them, at the cost of only being able to recolor a garment that's already there (it can't add a shirt where there is none, or change a collar/sleeve shape); graceful fallback states for permission-denied, no-camera-found, no-person-in-frame, and generic errors; "Add to cart" using the same `CartRepository` as Chat/Cart.
