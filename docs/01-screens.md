...
- **Size changes:** each line has a size picker (the product's `available_sizes` when known, otherwise a letter or UK-numeric range) — changing a size that already exists in the cart merges the lines (`CartRepository.changeSize`) — plus quantity and Remove.
- **Camera fit check (prototyped, removed):** a "Measure my size" popup that estimated chest size from the webcam (face/iris for scale, pose outline for shoulders and torso) and gave per-item fit verdicts, aimed at reducing returns. It is preserved in git history at commit `612bf7e` and removed because it was not reliable: scanning the same person in the same seat gave chest estimates of ≈ 105, 124 and 90 cm (a size step is ~5 cm). Causes seen on real frames: no absolute scale from a single webcam (face-size ruler ±5–10%), pose shoulder points misplaced by ~20 cm, person-outline gaps on dark striped shirts, arm position changing the outline by ~13%, and a seated laptop camera (0.5–0.8 m) showing only head, shoulders and ~14–20 cm of chest, so belly/chest — what actually decides T-shirt size — is out of view. Shoulder width alone cannot substitute (a true XXL read as XS–M). No tape-measured ground truth was available to calibrate against. Revisit only with tape-measured data for several people, a chest-in-view capture (~1 m) or a different modality, and probably the customer's known usual size as the anchor.
